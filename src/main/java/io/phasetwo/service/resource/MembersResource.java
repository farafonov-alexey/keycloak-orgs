...
public class MembersResource extends OrganizationAdminResource {
  ...
  public MembersResource(OrganizationAdminResource parent,
  ...
  public Stream<UserRepresentation>
  getMembers(@QueryParam("search") String searchQuery,
             @QueryParam("first") Integer firstResult,
             @QueryParam("max") Integer maxResults) {
    log.debugf("Get members for %s %s [%s]", realm.getName(),
               organization.getId(), searchQuery);
  ...
  public Long getMembersCount() {
    log.debugf("Get members count for %s %s", realm.getName(),
               organization.getId());
  ...
  public Response removeMember(@PathParam("userId") String userId) {
    ...
    log.debugf("Remove member %s from %s %s", userId, realm.getName(),
               organization.getId());
    ...
    if (member != null && organization.hasMembership(member)) {
      ...
      adminEvent.resource(ORGANIZATION_MEMBERSHIP.name())
  ...
  public Response getMember(@PathParam("userId") String userId) {
    log.debugf("Check membership %s for %s %s", userId, realm.getName(),
               organization.getId());
    UserModel member = session.users().getUserById(realm, userId);
    if (member != null && organization.hasMembership(member)) {
      return Response.noContent().build();
  ...
  public Response addMember(@PathParam("userId") String userId) {
    log.debugf("Add %s as member for %s %s", userId, realm.getName(),
               organization.getId());
    canManage();
  ...
  @Path("/with-roles")
  @Produces(MediaType.APPLICATION_JSON)
  public Stream<UserRepresentationWithRoles>
  getMembersWithRoles(@QueryParam("search") String searchQuery,
                      @QueryParam("first") Integer firstResult,
                      @QueryParam("max") Integer maxResults) {
    log.debugf("Get members for %s %s [%s]", realm.getName(),
               organization.getId(), searchQuery);
        ...
        .map(m -> {
          ...
          List<OrganizationRole> roles =
              organization.getRolesStream()
                  ...
                  .toList();
          return new UserRepresentationWithRoles().user(user).organizationRoles(
              roles);
        });
  }
